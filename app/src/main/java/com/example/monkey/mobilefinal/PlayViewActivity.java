...
import com.example.monkey.mobilefinal.audio.MusicController;
import com.example.monkey.mobilefinal.util.UtilTime;

import butterknife.BindView;
import butterknife.ButterKnife;
import butterknife.OnClick;

public class PlayViewActivity extends LinearLayout implements MusicController.IPlayerStatus{
    ...
    public void toMusic(){
        final ClickDialogActivity dialog = new ClickDialogActivity(this.getContext(), new ClickDialogActivity.IClickDialogEventListenerClick() {
            @Override
            public void onToStartup() {
                controller.position = 0;
                controller.play();
            }

            @Override
            public void onToHero(){
                controller.position = 1;
                controller.play();
            }

            @Override
            public void onToBattle(){
    ...
    public PlayViewActivity(final Context context) {
        super(context);
        this.mContext = (PlayPagerActivity)context;
        LayoutInflater inflater = LayoutInflater.from(context);
        view = inflater.inflate(R.layout.activity_play_view, this);
